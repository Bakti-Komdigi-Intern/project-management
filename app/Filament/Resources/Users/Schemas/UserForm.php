...
class UserForm
{
    ...
    TextInput::make('employee_email')
        ...
        ->required()
        ->unique(ignoreRecord: true, modifyRuleUsing: fn (Unique $rule) => $rule->whereNull('deleted_at'))
        ->suffix('@baktikomdigi.id')
        ->dehydrateStateUsing(fn ($state) => $state . '@baktikomdigi.id')
        ->afterStateHydrated(function ($component, $state) {
            if ($state && str_contains($state, '@baktikomdigi.id')) {
                $component->state(str_replace('@baktikomdigi.id', '', $state));
            }
        })
